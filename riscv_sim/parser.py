class Instruction:
  ...
  @staticmethod
  def reg_num(reg):
    return int(reg[1:]) if reg.startswith('x') else None
  ...
  def parser_file(file_path):
    ...
    for line in raw_lines:
      ...
      if opcode in ["ADD","SUB"]:
        ...
        rd=Instruction.reg_num(parts[1])
        offset, rs1_part = parts[2].split('(')
        rs1=Instruction.reg_num(rs1_part[:-1])
        offset=int(offset)
        instructions.append(Instruction(opcode,rd,rs1,imm=offset))
        ...
        instructions.append(Instruction(opcode,rs1=rs1,rs2=rs2,target=target))
